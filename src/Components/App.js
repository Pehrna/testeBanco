...
function Postagens2({ navigation, routes }) {

    console.log(routes)
    console.log(JSON.stringify(routes))

    //const {numero} = routes.params;
    const {numero} = routes.numero;
    // const {itemId} = escolha.itemId;
    //var doidera = JSON.stringify(escolha.numero);

    console.log({numero});
    console.log(JSON.stringify(numero))
    var rowsUser = [];
    //console.log(escolha.numero)
    //   for (let i = 0; i < projeto.length; i++) {       
    //       if (projeto[i].userId === escolha.numero) {
    //           rowsUser.push(projeto[i]);
    //       }
    //   }
      console.log(rowsUser)
